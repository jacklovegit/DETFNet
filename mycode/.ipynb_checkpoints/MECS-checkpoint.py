...
class MECS(nn.Module):
    def __init__(self, in_channels, out_channels, channel_attention_reduce=4):
        ...
        self.initial_depth_conv = nn.Conv2d(in_channels, out_channels, kernel_size=5, padding=2, groups=in_channels)

        self.depth_convs = nn.ModuleList([
            nn.Conv2d(out_channels, out_channels, kernel_size=(1, 7), padding=(0, 3), groups=out_channels),
            nn.Conv2d(out_channels, out_channels, kernel_size=(7, 1), padding=(3, 0), groups=out_channels),
            nn.Conv2d(out_channels, out_channels, kernel_size=(1, 11), padding=(0, 5), groups=out_channels),
            nn.Conv2d(out_channels, out_channels, kernel_size=(11, 1), padding=(5, 0), groups=out_channels),
            nn.Conv2d(out_channels, out_channels, kernel_size=(1, 21), padding=(0, 10), groups=out_channels),
            nn.Conv2d(out_channels, out_channels, kernel_size=(21, 1), padding=(10, 0), groups=out_channels),
        ])

        self.pointwise_conv = nn.Conv2d(out_channels, out_channels, kernel_size=1, padding=0)
        self.act = nn.GELU()
